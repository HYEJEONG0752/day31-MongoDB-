...
4. **MongoDB의 주요 특징 중 수평적 확장성에 대해 설명하고, 샤딩(Sharding)이 어떻게 수평적 확장을 지원하는지 서술하세요.**
    - 데이터베이스가 증가하는 데이터 양과 트래픽을 처리하기 위해 여러 서버에 걸쳐 데이터를 분산할 수 있는 능력을 의미합니다.
...
5. **MongoDB에서 BSON(Binary JSON) 형식이 사용되는 이유와 이 형식의 주요 특징에 대해 설명하세요.**
    ...
    -  **BSON 특징 :**
        - 이진형식 : 이진형식으로 데이터를 표현하여, JSON보다 더 빠르고 효율적으로 데이터를 처리할 수 있습니다.
        - 다양한 데이터 타입 : 기본 타입(문자열, 정수, 부동 소수점, 불리언, null), 복합타입(배열, 객체), 특수 타입(날짜, ObjectId 등)
        - 크기 정보 포함 : 각 필드의 크기를 포함하고 있어, 데이터의 크기를 쉽게 계산할 수 있습니다.
        - 계층적 구조 : 중첩된 문서와 배열을 지원하여, 복잡한 데이터 구조를 표현할 수 있습니다.
        - 인덱싱 지원 : MongoDB의 인덱싱 기능과 잘 통합되어 있어, 효율적인 쿼리 성능을 제공합니다.
        - 호환성 : JSON과 호환성이 있어, JSON형식으로 데이터를 쉽게 변환할 수 있고 이로 인해 다른 시스템과의 데이터 교환을 용이하게 합니다.
---
